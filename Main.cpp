...
void drawDisplay(bool drawNextPoint) {
  ...
  if (state.currentAppMode == MODE_IDLE) {
    ...
    left[0] = "Relays"; // run humidifier 100% if cooling
    leftColors[0] = GRAY_600;
    sprintf(buffer1, "%s", state.relayControl == 1 ? "On" : "Off"); right[0] = buffer1;
    rightColors[0] = _config2FieldColor(C2RelayControl);
